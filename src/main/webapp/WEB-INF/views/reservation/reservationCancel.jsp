...
.reservation-info h4, .cancle-reason h4 {
	margin-top: 4px;
	font-weight: bold;
	margin-bottom: 15px;
}
				...
				<div class="cancle-scroll">
					...
					<c:if test="${reservationDTO.reservationStatus ne 'CANCELLED'}">
						<form action="/reservations/${reservationDTO.reservationId }/cancel"
							...
							<label for="mind">단순변심</label> <input type="radio"
								class="disaster" name="cancleReason" value="천재지변"> <label
								for="disaster">천재지변</label> <input type="radio" class="moved"
								name="cancleReason" value="이벤트일정 변동"> <label for="moved">이벤트일정
								변동</label> <input type="radio" class="disease" name="cancleReason"
								value="기타사유"> <label for="disease">기타</label>
							<p>
								환불 금액 :
							...
							<div class="btn-box">
								...
								<button onclick="history.back()" id="btn-back">뒤로가기</button>
					</article>
