...
<body>
	...
	<c:set var="saved"
		value="${sessionScope.SPRING_SECURITY_SAVED_REQUEST}" />
	...
	<c:set var="isAdminMode"
		value="${param.mode eq 'admin' or savedIsAdmin}" />
		...
		<c:when test="${isAdminMode}">
			<div class="admin-wrap">
				<div class="card admin-card">
					...
					<form id="admin-form" action="<c:url value='/login'/>"
						method="post" autocomplete="on" novalidate>
						<div class="field">
							<label class="label" for="login_admin">아이디</label> <input
								class="input" type="text" id="login_admin" name="login" required />
						</div>
						<div class="field">
							<label class="label" for="password_admin">비밀번호</label> <input
								class="input" type="password" id="password_admin"
		...
		<c:otherwise>
			...
			<main class="container">
				...
				<div class="tabs" style="max-width: 520px; margin: 0 auto 10px;">
					<button type="button" class="tab-btn active" data-tab="tab-member">회원
						로그인</button>
					<button type="button" class="tab-btn" data-tab="tab-guest">비회원
						예약조회</button>
				</div>
				...
				<div class="card" style="max-width: 520px; margin: 0 auto;">
					<section id="tab-member" class="tab-panel active">
						...
						<form id="member-form" action="<c:url value='/login'/>"
							method="post" autocomplete="on" novalidate>
							<div class="field">
								<label class="label" for="login_uid">이메일 또는 아이디</label> <input
									class="input" type="text" id="login_uid" name="login"
									placeholder="이메일 또는 아이디" required />
							</div>
							<div class="field">
								<label class="label" for="password_uid">비밀번호</label> <input
									class="input" type="password" id="password_uid" name="password"
							...
							<div class="actions">
								...
								<a class="btn btn-ghost" href="<c:url value='/join'/>"
									style="text-decoration: none; display: inline-grid; place-items: center;">회원가입</a>
							</div>
							<div class="msg">
					...
					<section id="tab-guest" class="tab-panel">
						...
						<form id="guest-form"
							action="<c:url value='/guest/reservation/find'/>" method="post"
							autocomplete="off" novalidate>
							<div class="field">
								<label class="label" for="email_pw">이메일</label> <input
									class="input" type="text" id="email_pw" name="userEmail"
									placeholder="이메일" required />
							</div>
							<div class="field">
								<label class="label" for="reservation_no">예약번호</label> <input
									class="input" type="text" id="reservation_no"
									name="reservationNo" placeholder="예약번호" required />
							</div>
							<sec:csrfInput />
							<div class="actions">
								<button type="button" class="btn btn-ghost"
	...
	<script>
		...
		(function() {
			...
			form.addEventListener('submit', function(e) {
				...
				if (!login) {
					e.preventDefault();
					showMsg('member-msg', '아이디 또는 이메일을 입력해 주세요.');
					loginEl.focus();
					return;
				}
				if (!pwd) {
					e.preventDefault();
					showMsg('member-msg', '비밀번호를 입력해 주세요.');
					passEl.focus();
					return;
				}
				if (login.includes('@') && !isEmail(login)) {
					e.preventDefault();
					showMsg('member-msg', '올바른 이메일 형식이 아닙니다.');
					loginEl.focus();
					return;
				}
				if (pwd.length < 4) {
					e.preventDefault();
					showMsg('member-msg', '비밀번호는 4자 이상이어야 합니다.');
					passEl.focus();
					return;
				}
		...
		(function() {
			...
			form.addEventListener('submit', function(e) {
				...
				if (!login) {
					e.preventDefault();
					showMsg('admin-msg', '아이디를 입력해 주세요.');
					loginEl.focus();
					return;
				}
				if (!pwd) {
					e.preventDefault();
					showMsg('admin-msg', '비밀번호를 입력해 주세요.');
					passEl.focus();
					return;
				}
				if (login.includes('@') && !isEmail(login)) {
					e.preventDefault();
					showMsg('admin-msg', '올바른 이메일 형식이 아닙니다.');
					loginEl.focus();
					return;
				}
				if (pwd.length < 4) {
					e.preventDefault();
					showMsg('admin-msg', '비밀번호는 4자 이상이어야 합니다.');
					passEl.focus();
					return;
				}
		...
		(function() {
			...
			form.addEventListener('submit',
					function(e) {
						...
						if (!email) {
							e.preventDefault();
							showMsg('guest-msg', '이메일을 입력해 주세요.');
							emailEl.focus();
							return;
						}
						if (!isEmail(email)) {
							e.preventDefault();
							showMsg('guest-msg', '올바른 이메일 형식이 아닙니다.');
							emailEl.focus();
							return;
						}
						if (!no) {
							e.preventDefault();
							showMsg('guest-msg', '예약번호를 입력해 주세요.');
							noEl.focus();
							return;
						}
						if (!/^[A-Za-z0-9-]{4,32}$/.test(no)) {
							e.preventDefault();
							showMsg('guest-msg',
									'예약번호 형식이 올바르지 않습니다. (영숫자/하이픈 4~32자)');
							noEl.focus();
							return;
						}
